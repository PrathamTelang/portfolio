...
export default function Grid({ items = [] }: { items?: GridItem[] }) {
  ...
  return (
    ...
    >
      <div className="w-11/12 lg:w-11/12 lg:w-3/5 relative bg-white border-x border-lightborder dark:bg-darkbg dark:border-darkborder">
        ...
        <div className="pointer-events-none absolute inset-0 z-10">
          ...
          >
            {Array.from({ length: cols }).map((_, i) => (
              <div
                key={i}
                className="border-x border-lightborder  dark:border-darkborder"
              />
            ))}
          </div>
        </div>
